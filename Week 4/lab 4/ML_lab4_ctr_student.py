# Databricks notebook source exported at Tue, 21 Jul 2015 06:56:28 UTC
# MAGIC %md
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    nonZeroIndices = sorted([OHEDict[x] for x in rawFeats ])
    return SparseVector(numOHEFeats,nonZeroIndices,np.ones(len(nonZeroIndices)))
...
def parsePoint(point):
    ...
    """
    return [(i, j) for i, j in enumerate(point.split(',')[1:])]
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    """
    return LabeledPoint(point.split(',')[0],oneHotEncoding(parsePoint(point), ctrOHEDict, numCtrOHEFeats))
...
classOneFracTrain = OHETrainData.map(lambda lp:lp.label).reduce(lambda x,y:x+y)/(float(OHETrainData.count()))
print classOneFracTrain

logLossTrBase = OHETrainData.map(lambda lp:computeLogLoss(classOneFracTrain,lp.label)).reduce(lambda x,y:x+y)/(float(OHETrainData.count()))
...
def evaluateResults(model, data):
    ...
    """
    return (data.map(lambda lp:(lp.label,getP(lp.features,model.weights,model.intercept))).map(lambda x:computeLogLoss(x[1],x[0])).reduce(lambda x,y:x+y))/(float(data.count()))

logLossTrLR0 = evaluateResults(model0, OHETrainData)
#print logLossTrLR0.take(5)
...
# TODO: Replace <FILL IN> with appropriate code
logLossValBase = OHEValidationData.map(lambda lp:computeLogLoss(classOneFracTrain,lp.label)).reduce(lambda x,y:x+y)/(float(OHEValidationData.count()))
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    return LabeledPoint(point.split(',')[0],SparseVector(numBucketsCTR,hashFunction(numBuckets,parsePoint(point),printMapping=False)))
...
def computeSparsity(data, d, n):
    ...
    Returns:
        ...
        average sparsity =numzeros/total num features
    """
    numNonZeros = data.map(lambda lp:lp.features).map(lambda x:len(x.indices)).reduce(lambda x,y:x+y)
    
    return ((numNonZeros)/float(d))/float(n)
...
logLossTestBaseline = hashTestData.map(lambda lp:computeLogLoss(classOneFracTrain,lp.label)).reduce(lambda x,y:x+y)/(float(hashTestData.count()))
