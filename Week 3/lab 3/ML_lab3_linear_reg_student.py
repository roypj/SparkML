# Databricks notebook source exported at Mon, 13 Jul 2015 05:27:15 UTC
# MAGIC %md
...
def parsePoint(line):
    ...
    return LabeledPoint(parsedData[0],parsedData[1:])
...
averageTrainYear = (parsedTrainData
                    .map(lambda x:x.label).reduce(lambda x,y:x+y))/(float(parsedTrainData.count()))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda (x,y):squaredError(x,y)).reduce(lambda x,y:x+y)/(float(labelsAndPreds.count())))**0.5
...
def twoWayInteractions(lp):
    ...
    """
    featIter=itertools.product(lp.features,lp.features)
    twoWayInter=DenseVector([x*y for (x,y) in featIter]) 
    
              
    return LabeledPoint(lp.label,np.hstack((lp.features,twoWayInter)))
